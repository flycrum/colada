...
- **spec-d.ts override:** `files: ['**/*.spec-d.ts']` applies: (1) `@typescript-eslint/no-empty-object-type: 'off'` so type tests can assert empty object types (e.g. `expectTypeOf<None>().toEqualTypeOf<{}>()`); (2) `@typescript-eslint/no-unused-vars` with `varsIgnorePattern: '^_'` (and args/caughtErrors/destructuredArray `^_`) so intentionally unused type aliases for negative type tests can use a leading underscore (e.g. `type _Bad = ...`) without lint errors
