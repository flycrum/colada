...
- **sync script has no deps:** [scripts/sync-claude-marketplace.js](./scripts/sync-claude-marketplace.js) uses only Node built-ins; invoked by **git post-merge** ([.githooks/post-merge](./.githooks/post-merge)) after pull and via `pnpm run sync:claude-marketplace`. Enable hooks once per clone: `pnpm run setup:githooks`. Optional script test (not vitest): `pnpm run sync:test:claude-marketplace` runs [sync-claude-marketplace-test-all-conditions.js](./scripts/sync-claude-marketplace-test-all-conditions.js); does not run as part of `pnpm test`
