...
- **fast-json-patch** — RFC 6902; used by [.agents/plugins/agents-plugin-marketplaces/scripts/sync-claude-marketplace.js](./.agents/plugins/agents-plugin-marketplaces/scripts/sync-claude-marketplace.js) for surgical edits to `.claude-plugin/marketplace.json` and `.claude/settings.json`. Script invoked by **git post-merge** ([.githooks/post-merge](./.githooks/post-merge)) after pull and via `pnpm run marketplace-claude-sync`. Enable hooks once per clone: `pnpm run setup:githooks`. Optional script test (not vitest): `pnpm run marketplace-claude:test-sync`; does not run as part of `pnpm test`
