...
- **fast-json-patch** — RFC 6902; used by [scripts/sync-claude-marketplace.js](./scripts/sync-claude-marketplace.js) for surgical edits to `.claude-plugin/marketplace.json` and `.claude/settings.json`. Script invoked by **git post-merge** ([.githooks/post-merge](./.githooks/post-merge)) after pull and via `pnpm run sync:claude-marketplace`. Enable hooks once per clone: `pnpm run setup:githooks`. Optional script test (not vitest): `pnpm run sync:test:claude-marketplace`; does not run as part of `pnpm test`
