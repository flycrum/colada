...
description: Write or update Vitest type tests (*.spec-d.ts). Use when writing or updating type tests, or when asked to add type tests. Assert only on types from the API; use *.spec-d.ts; run pnpm test.
---
...
- Writing or updating `*.spec-d.ts` files
...
2. Use file extension `*.spec-d.ts`
