...
5. Keep rules/commands/skills DRY and lean per [writing-rules-commands-skills.md](./writing-rules-commands-skills.md)
6. Every README in plugin: ## Purpose only. Keep content succinct; sacrifice grammar for concision
7. Self-reference this plugin (\_plugin-example) when creating or updating any plugin
8. Sync script picks up new plugins automatically: `.agents/plugins/agents-plugin-marketplaces/scripts/marketplace-claude-sync.js` scans `.agents/plugins/*/.claude-plugin/plugin.json`; no hand-edit of marketplace list needed. **After adding or modifying a plugin, run the sync script** so the marketplace and settings stay in sync: `pnpm run marketplace-claude:sync` from repo root. See [agents-plugin-marketplaces/rules/sync-after-plugin-changes.md](../../agents-plugin-marketplaces/rules/sync-after-plugin-changes.md) and [agents-plugin-marketplaces](../../agents-plugin-marketplaces)
