...
8. Sync script picks up new plugins automatically: `scripts/sync-claude-marketplace.js` scans `.agents/plugins/*/.claude-plugin/plugin.json`; no hand-edit of marketplace list needed. **After adding or modifying a plugin, run the sync script** so the marketplace and settings stay in sync: `pnpm run sync:claude-marketplace` from repo root. See [agents-plugin-marketplaces/rules/sync-after-plugin-changes.md](../../agents-plugin-marketplaces/rules/sync-after-plugin-changes.md) and [agents-plugin-marketplaces](../../agents-plugin-marketplaces)
