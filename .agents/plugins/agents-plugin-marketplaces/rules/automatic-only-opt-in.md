...
- Postinstall runs [sync-claude-marketplace.js](../../scripts/sync-claude-marketplace.js); creates/updates `.claude-plugin/marketplace.json` and `.claude/settings.json` with all discovered plugins. No hand-maintained list, no `/plugin marketplace add` or `/plugin install`
