...
Get the Colada plugin marketplace (colada-plugins) and all its plugins available in Claude Code with **no manual slash commands**. The only manual step is opting in via env; the rest is driven by postinstall and generated config.
...
2. **Install:** From repo root run `pnpm install`. The postinstall script runs and creates/updates:
...
4. **New plugins:** Adding a new plugin under `.agents/plugins/<name>/` with a `.claude-plugin/plugin.json` is enough. The next `pnpm install` (or postinstall run) will include it in the marketplace and in `enabledPlugins`; no developer action required.
