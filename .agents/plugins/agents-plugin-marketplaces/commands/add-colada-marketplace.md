...
2. **Githooks (once per clone):** From root run `pnpm run setup:githooks`. Post-merge then runs [marketplace-claude-sync.js](../scripts/marketplace-claude-sync.js) after git pull (merge). If you haven't pulled yet, run `pnpm run marketplace-claude:sync` once.
