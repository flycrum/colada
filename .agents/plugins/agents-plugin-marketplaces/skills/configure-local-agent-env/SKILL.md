...
3. **If they enabled Claude:** After setting `ENABLE_LOCAL_AGENT_CLAUDE=true`, they should run **`pnpm run setup:githooks`** once (so the post-merge hook runs after pull), then **`pnpm run marketplace-claude-sync`** once to create/update `.claude-plugin/marketplace.json` and `.claude/settings.json`. After that, every `git pull` (when it merges) runs the sync. Opening the project in Claude Code uses that config—no `/plugin marketplace add` or `/plugin install` steps. If they disabled Claude, run `pnpm run marketplace-claude-sync` for clean slate.
