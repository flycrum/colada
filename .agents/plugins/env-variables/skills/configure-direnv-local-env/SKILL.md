...
2. **Local config workflow:** Repo has a committed default [.env](../../../../../../.env). To override, copy root [.env.example](../../../../../../.env.example) to **.envrc.local** at repo root (gitignored; never commit). Edit the `export ENABLE_LOCAL_AGENT_*` lines. Root [.envrc](../../../../../../.envrc) loads `.env` first, then `.envrc.local` when present. Variables and semantics: [variables.md](../../rules/variables.md), [where-to-define.md](../../rules/where-to-define.md).
