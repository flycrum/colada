...
- **ENABLE_LOCAL_AGENT_CURSOR**: `true` or `false`; when true, sync script populates `.cursor/commands` and `.cursor/rules` from `.agents/plugins` via symlinks; when false, removes only our symlinks and manifest (clean slate). See [marketplace-cursor-sync.js](../../agents-plugin-marketplaces/scripts/marketplace-cursor-sync.js)
- **CURSOR_EXCLUDED_PLUGINS**: optional; comma-separated plugin dir names to exclude from Cursor sync (e.g. `env-variables,turborepo`). Only applied when `ENABLE_LOCAL_AGENT_CURSOR=true`; when false, ignored so you can keep the same line when toggling
